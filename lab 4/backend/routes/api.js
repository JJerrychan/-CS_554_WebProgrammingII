...
router.get('/characters/page/:pagenum', async (req, res) => {
  try {
    req.params.pagenum = utils.checkPage(req.params.pagenum);
  } catch (e) {
    return res.status(404).json({ error: e });
  }
  try {
    res.json(await data.getCharactersByPage(req.params.pagenum));
  } catch (e) {
    res.status(500).json({ error: e });
  }
});
...
router.get('/comics/page/:pagenum', async (req, res) => {
  ...
  try {
    res.json(await data.getComicsByPage(req.params.pagenum));
  } catch (e) {
    res.status(500).json({ error: e });
  }
});
...
router.get('/stories/page/:pagenum', async (req, res) => {
  ...
  try {
    res.json(await data.getStoriesByPage(req.params.pagenum));
  } catch (e) {
    res.status(500).json({ error: e });
  }
});
